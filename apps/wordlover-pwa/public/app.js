...
import { dictionaryStorageKeys, resolveDictionaryAssetUrl, resolveDictionaryConfig } from "./dictionary-config.js?v=20260714-9";
import { userSelectableDictionaries } from "./dictionary-registry.js?v=20260714-9";
import { dictionaryRecordMetadata, readSelectedDictionaryId, saveSelectedDictionaryId } from "./dictionary-selection.js?v=20260714-9";
import {
  formatDomainSuffix,
...
} from "./dictionary-rendering.js?v=20260714-9";
...
import {
  ...
  deriveKek,
} from "./persistence.js?v=20260714-9";

import {
  ratingFromRetries,
  spellingThreshold as _spellingThreshold,
} from "./spelling.js?v=20260714-9";
...
import { renderOnlineDictionaryActions } from "./online-dictionary-actions.js?v=20260714-9";
import { createUpdateManager, formatUpdateStatus } from "./update-manager.js?v=20260714-9";
import {
  createDictionarySupplementStore,
  mergeDictionarySupplementRecords,
  normalizeSupplementTerm,
} from "./dictionary-supplements.js?v=20260714-9";
import { validateYoudaoEntry } from "./youdao-entry-schema.js?v=20260714-9";
import { appendSupplementHint, quizMeaningWithSupplement, savedSupplementToStudySnapshot } from "./study-supplements.js?v=20260714-9";
import { renderYoudaoState } from "./online-dictionary-result-renderer.js?v=20260714-9";
...
} from "./review-state.js?v=20260714-9";
...
} from "./full-dictionary.js?v=20260714-9";
...
const repairAppShellButton = document.querySelector("#repairAppShell");
...
const APP_VERSION = "0.6.2-product.20260714-9-v165";
...
// only — it never drives the update prompt (APP_VERSION does), so a redeploy of
// identical shell code does not nag users to "Apply update".
const BUILD_STAMP = "dev";
const USER_DATA_FORMAT_VERSION = "0.4";
const SHELL_CACHE_VERSION = "wordlover-shell-v165";
...
window.WordLoverDictionarySupplements = Object.freeze({
  ...
  upsertFromLookup: async (entry, metadata = {}) => { const record = await dictionarySupplements.upsertFromLookup(entry, metadata); savedStudySupplements.set(record.normalizedTerm, record); announceDictionarySupplementChange("saved", record, record.providerId); return record; },
...
function isMemorizeReviewMode(mode) {
  return mode === "review" || mode === "practice";
}

function renderReviewDictionaryButton(mode) {
  if (!isMemorizeReviewMode(mode)) return "";
  const visible = Boolean(activeQuiz?.reviewDictionaryVisible);
  return `<button type="button" class="secondary-button review-dictionary-toggle" data-review-dictionary-toggle="1" aria-expanded="${visible}" aria-controls="reviewDictionaryDefinition">${visible ? "Hide" : "Show"}</button>`;
}
...
function renderVocabulary() {
  ...
  if (!stats.active.length && !stats.archived.length) {
    ...
    vocabularyList.innerHTML = isKnown
      ...
      : isSpelling
        ...
        : `${renderVocabularyFilter(query)}<p class="muted">Search a word and add it to your Memorize list, or set On Return to "Add to Memorize list".</p>`;
    return;
  }
  ...
  vocabularyList.innerHTML = vocabularyView.filter === "summary"
    ? `${statsHtml}${renderVocabularyFilter(query)}`
...
function getTodayStats() {
  ...
  const { reviewed, practiced } = countTodayReviewActivity(studyEvents);
  ...
  return { newSaved, reviewed, practiced, mastered, known, dueCount: getDueVocabularyItems().length, dueTodayCount: getDueTodayVocabularyItems().length, activeCount: getPracticeVocabularyItems().length };
}

function getSpellingTodayStats() {
  const newSaved = spellingItems.filter((item) => isToday(item.savedAt)).length;
  const { reviewed, practiced } = countTodayReviewActivity(spellingEvents);
  const mastered = spellingItems.filter((item) => isToday(item.review?.masteredAt)).length;
  return { newSaved, reviewed, practiced, mastered, known: 0, dueCount: getDueSpellingItems().length, dueTodayCount: getDueTodaySpellingItems().length, activeCount: spellingItems.filter((item) => !item.archivedAt).length };
}
...
function renderStudyStats() {
  ...
  if (statPracticed) statPracticed.textContent = String(stats.practiced);
...
function renderStudyOneMoreLoading() {
  quizPanel.hidden = false;
  quizPanel.innerHTML = `
    <div class="study-one-more-card" aria-busy="true">
      <div class="quiz-question">
        <span>Study one more</span>
        <p class="muted">Finding a word...</p>
      </div>
    </div>
  `;
  quizPanel.scrollIntoView({ behavior: "smooth", block: "nearest" });
...
function renderQuizQuestionMarkup(entry, mode, optionsRevealed) {
  const reviewDictionaryVisible = isMemorizeReviewMode(mode) && Boolean(activeQuiz?.reviewDictionaryVisible);
  const localDefinition = reviewDictionaryVisible ? lookupTerm(entry.term) : null;
  return `
    <div class="quiz-question">
...
function reviewModeForItem(item) {
  const dueMs = Date.parse(item?.review?.dueAt ?? "");
  return Number.isFinite(dueMs) && dueMs <= appNowMs() ? "review" : "practice";
}

async function practiceVocabularyTerm(term) {
  const item = getVocabularyItem(term);
  if (!item || item.archivedAt) return false;
  ...
  renderStudyStats();
  renderQuiz(quizEntryFromVocabulary(item), activeVocabularyReviewSession.mode);
  studyPanel?.scrollIntoView({ behavior: "smooth", block: "start" });
  void persistUiPreferences();
  return true;
}
...
async function applyAppUpdate({ reload = true } = {}) {
  return updateManager.apply({ reload });
}
...
result.addEventListener("click", (event) => {
  if (event.target instanceof HTMLButtonElement && event.target.id === "saveCurrentTerm" && currentResult) {
    const item = getVocabularyItem(currentResult.term);
    if (item && !item.archivedAt) {
      void practiceVocabularyTerm(currentResult.term);
    } else {
  ...
  if (event.target instanceof HTMLButtonElement && event.target.id === "addToSpelling" && currentResult) {
    const item = getSpellingItem(currentResult.term);
    if (item && !item.archivedAt) {
      void practiceSpellingTerm(currentResult.term);
    } else {
...
quizPanel.addEventListener("click", (event) => {
  ...
  const reviewDictionaryToggle = target.closest("[data-review-dictionary-toggle]");
  if (reviewDictionaryToggle instanceof HTMLButtonElement && activeQuiz && isMemorizeReviewMode(activeQuiz.mode)) {
    activeQuiz.reviewDictionaryVisible = !activeQuiz.reviewDictionaryVisible;
    const panel = quizPanel.querySelector("#reviewDictionaryDefinition");
    reviewDictionaryToggle.textContent = activeQuiz.reviewDictionaryVisible ? "Hide" : "Show";
    reviewDictionaryToggle.setAttribute("aria-expanded", String(activeQuiz.reviewDictionaryVisible));
...
updateManager = createUpdateManager({
  pageRelease: { appVersion: APP_VERSION, buildId: BUILD_STAMP, shellCache: SHELL_CACHE_VERSION },
  persist: async () => {
    await Promise.all([persistVocabulary(), persistStudyEvents(), persistSpelling(), persistSpellingEvents(), persistUiPreferences()]);
  },
  confirmRepair: () => Promise.resolve(window.confirm("Repair the WordFan app shell? This removes only WordFan service-worker caches. Vocabulary, study history, dictionaries, tracks, and preferences are preserved.")),
  onState: (state) => {
    updateStatus.textContent = formatUpdateStatus(state);
    applyUpdateButton.disabled = !state.waitingWorker || state.phase === "activating";
    repairAppShellButton.disabled = state.phase === "checking" || state.phase === "activating";
  },
});
...
repairAppShellButton.addEventListener("click", () => {
  void updateManager.repair();
});

window.addEventListener("online", () => void updateManager.check());
let updateVisibilityCheckedAt = 0;
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible" && Date.now() - updateVisibilityCheckedAt > 30 * 60 * 1000) {
    updateVisibilityCheckedAt = Date.now();
    void updateManager.check();
  }
});
window.setInterval(() => void updateManager.check(), 5 * 60 * 60 * 1000);
