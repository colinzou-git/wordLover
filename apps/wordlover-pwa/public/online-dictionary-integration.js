...
function hydrate(element) {
  ...
  const controller = createOnlineDictionaryLookupController({ provider: youdaoProvider, mode: element.dataset.mode ?? "manual", getSaved: async (term, providerId) => (await supplements?.get(term, providerId))?.entry ?? null, allowSessionCache: false, onState: render });
...
window.addEventListener("wordlover:supplement-changed", (event) => {
  const { normalizedTerm, providerId } = event.detail ?? {};
  ...
  document.querySelectorAll(".online-dictionary-actions[data-online-dictionary-provider='youdao']").forEach((element) => {
    if (normalizeSupplementTerm(element.dataset.term) === normalizedTerm) void controllers.get(element)?.lifecycle.display(element.dataset.term);
  });
});
