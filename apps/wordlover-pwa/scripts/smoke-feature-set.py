...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        page.goto(f"{base}/?fresh=v62", wait_until="domcontentloaded")
        ...
        if state.get("appVersion") != "0.6.2-product.20260528-v62":
            failures.append(f"appVersion not v62: {state.get('appVersion')}")
        if state.get("shellCacheVersion") != "wordlover-shell-v62":
            failures.append(f"shell cache version not v62: {state.get('shellCacheVersion')}")
