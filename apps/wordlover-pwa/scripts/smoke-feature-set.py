...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        page.goto(f"{base}/?fresh=v51", wait_until="domcontentloaded")
        ...
        if state.get("appVersion") != "0.6.2-product.20260527-v51":
            failures.append(f"appVersion not v51: {state.get('appVersion')}")
        if state.get("shellCacheVersion") != "wordlover-shell-v51":
            failures.append(f"shell cache version not v51: {state.get('shellCacheVersion')}")
